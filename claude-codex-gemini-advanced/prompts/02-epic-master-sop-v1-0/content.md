...
6. **Check parent epic completion** — a phase is ONLY complete when **every** sub-epic under it is in `Done` or `Archive`:
     ...
     d) ONLY when ALL sub-epics are `Done` or `Archive` → move Parent Epic to `Review` state.
7. **After completing a parent epic, immediately find the next work.** Do NOT stop or ask the user. Check in this order:
     ...
     d) `devchain_list_epics(statusName=Backlog)` — any `Backlog` items? → run **Backlog Review** (Section 6.2) to send them through planning, then REPEAT from step 2 when new epics arrive.
     e) ONLY when **all of the above return empty** → proceed to step 8.
8. ONLY after confirming **no** epics in `New`, `Draft`, `In Progress`, `Review`, `QA`, or `Backlog` exist: request code review — use devchain_list_agents to identify the agent responsible for code review and send a message to review all completed epics.
9. **After code review completes** and no open work remains → run **Backlog Review** (Section 6.2) one final time to check for items generated by the code review.
